...
import { Plus, Send, Pause, Archive, Trash2, RotateCw } from 'lucide-react';
...
export const CampaignManager = () => {
  ...
  const handleDeleteCampaign = async (campaignId: string, campaignName: string) => {
    if (!window.confirm(`Are you sure you want to delete the campaign "${campaignName}"?`)) return;
    try {
      const { error } = await supabase.from('campaigns').delete().eq('id', campaignId);
      if (error) throw error;
      await supabase.from('audit_logs').insert({ user_id: user?.id, action: 'Campaign Deleted', entity_type: 'campaign', entity_id: campaignId, details: { name: campaignName } });
      loadCampaigns();
    } catch (error: any) {
            ...
            <div key={campaign.id} className="flex flex-col bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              {canManage && (
                <div className="flex gap-2 pt-4 border-t border-gray-100">
                  ...
                  <button onClick={() => handleDeleteCampaign(campaign.id, campaign.name)} className="flex items-center justify-center gap-1 px-3 py-2 text-sm bg-red-100 text-red-800 font-semibold rounded-lg hover:bg-red-200"> <Trash2 size={16} /> </button>
                </div>
              )}
            </div>
          ))
